...
export default function PerfilPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (!user) return;
      setEmail(user.email ?? '');
      setNome((user.user_metadata?.full_name as string | undefined) ?? '');
      const raw = (user.user_metadata?.telefone as string | undefined) ?? '';
      setTelefone(raw ? formatTelefone(raw) : '');
      setLoading(false);
    });
  }, []);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              required
              value={nome}
              onChange={(e) => { setNome(e.target.value); setError(''); setSuccess(false); }}
          ...
          <div>
            ...
            <input
              ...
              required
              value={telefone}
              onChange={(e) => { setTelefone(formatTelefone(e.target.value)); setError(''); setSuccess(false); }}
